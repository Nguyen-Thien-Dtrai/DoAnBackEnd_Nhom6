...
class OrderDetail extends Db
{
    ...
    {  
        $sql = self::$connection->prepare("UPDATE ordersdetail SET quantity = ?, price = ? WHERE productid = ? AND orderid = ?");  
        if (!$sql) {  
            die("Lỗi chuẩn bị câu lệnh: " . self::$connection->error);  
        }  
        $sql->bind_param('iiii', $quantity, $price, $productId, $orderId);  
        return $sql->execute();  
    }  
    ...
    {  
        $sql = self::$connection->prepare("DELETE FROM ordersdetail WHERE orderid = ?");  
        if (!$sql) {  
            die("Lỗi chuẩn bị câu lệnh: " . self::$connection->error);  
        }  
        $sql->bind_param('i', $orderId);  
